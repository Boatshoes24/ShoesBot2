...
const { bosses, num_bosses } = require('./amirdrassil.json');
const spaceChar = '\u200B';
const commandName = 'amirdrassil';
...
function formatEmbedArgs(args) {
	...
	if (!args.length) {
		...
		embedFields.push({
			name: 'All Auras',
			value: `!${commandName} all\nAuras: ${auraTotalCount}`,
			inline: false,
		});
...
module.exports = {
	name: `${commandName}`,
	aliases: ['adh', 'amir', 'drassil'],
	cooldown: 1,
	execute(msg, args) {
		let title = 'Amirdrassil Auras';

		const amirdrassilEmbed = new Discord.MessageEmbed()
			.setTitle(title)
			.setThumbnail(
				'https://dotesports.com/wp-content/uploads/2023/03/WoW-Aberrus-the-Shadowed-Crucible.jpg'
			)
			.setTimestamp()
			.setFooter({
		...
		if (addEmbeds[0] !== false) {
			amirdrassilEmbed.addFields(addEmbeds);
		}
		let addDescription = formatDescription(args);
		if (addDescription !== 'false') {
			amirdrassilEmbed.setDescription(addDescription);
		}

		msg.channel.send({ embeds: [amirdrassilEmbed] });
	},
};
